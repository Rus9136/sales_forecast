...
### 3.5 Post-processing (P1-5) — `aa4d604`

Последний код-фикс из P1. Четыре части в `forecast_postprocessing_service.py`:
1. **Двойное сглаживание:** второй проход (postproc) сравнивал прогноз с общим 7-дневным средним и клиппил ±50% — у точек с недельностью пятница/выходной срезались как «скачок» (поверх DOW-aware сглаживания агента). Теперь база — норма того же дня недели (8 недель).
2. **Множители убраны:** ×1.1 (кофейни-выходные), ×1.15 (близость праздника) — рудименты вредного weekend boost. Остались floor/ceiling.
3. **CI → split-conformal:** эвристику (normal × CV-cap 0.5) заменил эмпирическими относительными остатками из `forecast_accuracy_log`. Проверено в проде: `split_conformal`, 25 остатков, честный интервал 243k..1055k вокруг 800k.
4. **Anomaly DOW-aware:** z-score против нормы своего дня недели — выходные не помечаются аномалией системно.

Тесты `test_postprocessing_p1_5.py` (6). **620 passed.**

---

## Статус по ML_AUDIT_REPORT.md

**P0 (6/6):** ✅ все. **P1 (8/8 код-фиксы):** ✅ P1-2,3,5,6,7 закрыты; 🟡 P1-1 (is_outlier_day — метрики честные, фича в модели оставлена по решению плана), P1-8 (tweedie-эксперимент + intermittent-метрики; smearing не добавлен), P1-4 (стал измеримым через horizon_days; hybrid-модели — data/product-работа).

**P2 — улучшения точности:** требуют внешних данных/продукта, вне код-скоупа аудита: hybrid horizon, банкеты, погода, квантильные интервалы модели (не путать с conformal-CI post-processing — тот сделан), two-stage SKU, каннибализация, разметка 91/91 точек. Roadmap аудита ранжирует их ПОСЛЕ корректностной части — она закрыта.
