...
function outcomeTooltip(o: PriceOutcome): string {
  ...
  // Наличие товара: без этой строки «продажи упали» и «продавать было нечего»
  // выглядят одинаково.
  const oos = (o.days_no_stock_before ?? 0) + (o.days_no_stock_after ?? 0)
  if (o.days_no_stock_after == null) {
    lines.push('Наличие товара не проверялось: за этот период нет снимков складских остатков.')
  } else if (oos > 0) {
    lines.push(
      `Товара не было ${oos} ${oos === 1 ? 'день' : oos < 5 ? 'дня' : 'дней'} `
      + `(${o.days_no_stock_before ?? 0} до изменения, ${o.days_no_stock_after} после). `
      + 'Эти дни из расчёта выброшены: продажи в них упали не из-за цены.',
    )
  } else {
    lines.push('Товар был на складе все дни обоих окон — дефицитом результат не объясняется.')
  }
  return lines.join('\n')
}
