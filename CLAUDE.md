...
## Recent Updates (2025-06-24 & 2025-06-26)
...
#### Модель v2.0 (180 дней, с нуля):
- **Данные**: 6,008 записей (⚠️ расследовать несоответствие с БД)
- **Test MAPE**: 7.67% ⭐ **ЛУЧШИЙ РЕЗУЛЬТАТ**
- **Validation MAPE**: 28.46%
- **R²**: 0.9952
- **Разделение**: 70% train / 15% validation / 15% test
- **Важные признаки**: pct_change_1d, lag_1d_sales, rolling_7d_std_sales
...
- **ML Model v2.0**: ✅ Модель с train/val/test split на 6-месячных данных
  - **Test MAPE: 7.67%** - лучший результат в истории проекта
  - **Validation MAPE: 28.46%** - контроль обучения
  - **R²: 0.9952** - превосходная точность модели
  - **6,008 записей** - обучение на максимальном объеме данных
...
## ⚠️ КРИТИЧЕСКАЯ ПРОБЛЕМА ДЛЯ РАССЛЕДОВАНИЯ

### 🔍 Несоответствие данных модели (2025-06-26)
**ПРОБЛЕМА**: Модель использует 6,008 записей, но в БД за 180 дней только 5,281 запись.

#### 📊 Фактические данные в БД (sales_summary):
- **Общее количество записей**: 15,031
- **За последние 180 дней**: 5,281 запись (с total_sales > 0)
- **Период**: 2024-12-28 до 2025-06-26 (181 день)
- **Департаментов**: 39 активных
- **Среднее в день**: ~29 записей (5,281 ÷ 181)

#### 🤖 Данные, используемые моделью:
- **Train**: 3,505 записей (70%)
- **Validation**: 751 запись (15%)
- **Test**: 752 записи (15%)
- **Всего**: 6,008 записей ❌ **НЕ СООТВЕТСТВУЕТ БД**

#### 🔍 Возможные причины:
1. **TrainingDataService** может загружать данные за период больше 180 дней
2. **Rolling features** могут влиять на количество записей
3. **Фильтрация данных** в prepare_training_data может работать некорректно
4. **Кэширование** старых данных в процессе обучения

#### 📋 TODO для расследования:
- [ ] **Проверить логику TrainingDataService.prepare_training_data()**
- [ ] **Добавить детальное логирование периода загружаемых данных**
- [ ] **Проверить фактические даты, используемые моделью**
- [ ] **Убедиться, что модель использует именно последние 180 дней**
- [ ] **Провести audit trail данных от БД до модели**
...
docker exec sales-forecast-db psql -U sales_user -d sales_forecast -c "
  ...
  FROM sales_summary 
  WHERE total_sales > 0 AND date >= CURRENT_DATE - INTERVAL '180 days';"
```
...
- [x] ✅ Увеличить период обучения модели до 6 месяцев
- [ ] **🔥 ПРИОРИТЕТ: Расследовать несоответствие 6,008 vs 5,281 записей**
