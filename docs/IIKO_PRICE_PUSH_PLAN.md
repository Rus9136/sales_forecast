...
| `IIKO_PRICE_PUSH_DEPARTMENTS` (env, CSV UUID) | Белый список точек. Пусто → разрешены все. С 2026-08-06 пуст: при статусе `NEW` черновик безвреден. Обязателен к возврату перед `PROCESSED` |
...
### 8.5 Этап 4 — пилотная эксплуатация (1–2 недели) — ЗАПУЩЕН 2026-08-06

Готовность на 2026-08-06 (Мадлен 18 мкр): 33 новые рекомендации свежего батча,
**все 33 проходят проверки отправки** (базис совпадает с каталогом), суммарный
ожидаемый эффект +52 448 ₸/нед. Лимит правила `max_changes_per_cycle` — 15
изменений за 14 дней, использовано 0, значит в первый приказ войдёт максимум 15.

Холостая сборка payload на живых данных (только GET в iiko): 6 действующих
приказов прочитано, атрибуты перенесены для всех 15 позиций, документ собрался
в ту же форму, что и приказы, заводимые вручную.

Утверждение и отправку делает заказчик через UI (решение 2026-08-06): вкладка
«Рекомендации» → отметить позиции → «Утвердить» → «Отправить в iiko», дата
вступления в силу — следующий день (подставляется по умолчанию).

Что смотреть после первого приказа:
1. Вкладка «Приказы в iiko» — документ в статусе «Отправлен», номер из iiko.
2. Управляющий проводит приказ в бэк-офисе (приходит черновиком).
3. Ночью 03:20 синк каталога → 03:25 сверка приказов → позиции получают статус
   «Применена» с `applied_at` = дата вступления приказа в силу.
4. Через 14 дней — `price_recommendation_outcome` и строка в «Эффекте решений».
...
* UI: вкладка «Приказы в iiko» (журнал, сверка, отмена) + диалог «Отправить в iiko»
  на «Рекомендациях».
* Конфигурация в `.env.prod`: `IIKO_PRICE_PUSH_ENABLED=True`,
  `IIKO_PRICE_ORDER_STATUS=NEW` (приказ-черновик), белый список **пуст —
  черновики может создавать любая точка** (решение заказчика 2026-08-06).
  Безопасно ровно потому, что статус `NEW`: документ ни на что не влияет, пока
  управляющий не проведёт его в бэк-офисе. **Перед переключением на `PROCESSED`
  белый список вернуть** — иначе автопроведение включится сразу по всей сети.
